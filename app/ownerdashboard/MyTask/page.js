...
useEffect( () => {
  ...
  try{
    const response = await fetch(`${baseUrl}/api/MainServices/GetAllTaskPost`, {
			method: "GET",
			headers: {
 ...
 {selectedTask && (
   ...
   <div className={styles.popupOverlay} onClick={() => setSelectedTask(null)}>
          <div className={styles.popupMenu} onClick={(e) => e.stopPropagation()}>
            <p><strong>Title:</strong> {selectedTask.JobTitle}</p>
            <p><strong>Platform:</strong> {platform[selectedTask.Platform]}</p>
            <p><strong>Instructions:</strong> {selectedTask.Instructions}</p>
            <p><strong>Required Proof:</strong> {proofTypes[selectedTask.RequiredProof]}</p>
